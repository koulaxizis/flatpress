...
{html_form}
{assign var="HOST_URI" value=$flatpress.HOST_URI|default:$panelstrings.blogurl}
...
<h2> {$panelstrings.gensetts} </h2>
	...
	<dd><select name="admin_email" id="admin_email">
            ...
            <option value="noreply@{$HOST_URI}">noreply@{$HOST_URI}</option>
            <option value="{$flatpress.author}@{$HOST_URI}">{$flatpress.author}@{$HOST_URI}</option>
        </select></dd>
