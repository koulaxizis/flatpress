    ...
    if(!empty($_POST)) {    
        ...
        if($arr=comment_validate()) {
            ...
            if ($fp_config['general']['notify'] && !user_loggedin()) {
                ...
                global $post;
                $admin_email = $_SERVER['SERVER_NAME'];
                $admin_email = "admin@" . preg_replace('#^https?://#', '', rtrim($admin_email,'/'));
                $comm_mail = isset($arr['email'])? "<{$arr['email']}>" : '';
                $from_mail =  isset($fp_config['general']['admin_email']) ?
                $fp_config['general']['admin_email'] : // like administrator@myblog.com or noreply@abzblog.info
                $admin_email;
                ...
                @utils_mail($from_mail, "{$fp_config['general'][title]} - {$arr['name']} posted a new comment", $mail);
