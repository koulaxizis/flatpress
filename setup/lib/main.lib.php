...
function getstep(&$id) {
	...
	if (!file_exists(LOCKFILE)) {
		...
		if (!file_exists(SETUPTEMP_FILE)) {
			...
			if ($x[0]!=$setupid)
				die('Η εγκατάσταση "τρέχει": αν έχεις πρόσβαση στον διακομιστή, μπορείς να διαγράψεις το ' .
				 SETUPTEMP_FILE .' για να ξεκινήσεις ξανά, διαφορετικά επικοινώνησε με τον διαχειριστή.');
			$i = intval($x[1]);	
		}
...
function validate() {
	if (!ctype_alnum($_POST['fpuser']))
		$err[] = "Το {$_POST['fpuser']} δεν είναι έγκυρο όνομα χρήστη. 
		Χρησιμοποίησε λατινικούς χαρακτήρες και/ή αριθμούς (χωρίς κενά).";
	
	if (strlen(trim(($_POST['fppwd']))) < 6)
		$err[] = "Ο κωδικός πρέπει να αποτελείται από τουλάχιστον 6 χαρακτήρες και να μην περιέχει κενά.";
	
	if (($_POST['fppwd'])!=($_POST['fppwd2']))
		$err[] = "Οι κωδικοί δεν είναι ίδιοι!"; 
	
	if (!(preg_match('!@.*@|\.\.|\,|\;!', $_POST['email']) ||
			preg_match('!^.+\@(\[?)[a-zA-Z0-9\.\-]+\.([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$!', $_POST['email'])))
		$err[] = "Το {$_POST['email']} δεν είναι έγκυρο ηλ. ταχυδρομείο.";
	
	$www = $_POST['www'];
	if (!(preg_match('!^http(s)?://[\w-]+\.[\w-]+(\S+)?$!i', $www)
    		|| preg_match('!^http(s)?://localhost!', $www)))
			$err[] = "Το $www δεν είναι έγκυρη διεύθυνση.";
...
function print_err() {
	global $err;
	if (isset($err)) {
		echo "<p><big>Σφάλμα!</big> 
		Προέκυψαν τα ακόλουθα σφάλματα:</p><ul>";
		foreach ($err as $val) {
			echo "<li>$val</li>";
		}
		echo "</ul>";
